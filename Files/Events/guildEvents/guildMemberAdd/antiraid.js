	...
	async execute(member, user) {
		cache.map(o => o).forEach(o => {
			o.joined.forEach(join => {
				if (join.join - Date.now() > antiraidSettings.maxInterval) cache.set(o.guild, {guild: o.guild, joined: cache.get(o.guild).joined.splice(cache.get(o.guild).joined.indexOf(join.id), 1)});
			});
