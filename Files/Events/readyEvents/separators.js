...
module.exports = {
	async execute() {
		const res = await client.ch.query('SELECT * FROM roleseparatorsettings WHERE stillrunning = $1;', [true]);
		res.rows.forEach(async (row) => {
			...
			if (guild) {
				...
				msg.client = client, msg.author = client.user, msg.guild = guild, msg.lanSettings = language.commands.settings, msg.lan = msg.lanSettings.separators, msg.m = message;
